...
** shrink-whitespace
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (global-set-key [backspace]
                     (lambda ()
                       (interactive)
                       (if (or (string-match "[\s-]" (char-to-string
                                                      (preceding-char)))
                               (bolp))
                           (call-interactively 'shrink-whitespace)
...
** CEDET
   ...
   #+end_src
** Highlight changes to buffer before save
   Like this section says, I want to have the text different until a
   save. So then the user remembers to save and not be a dingus.
   #+begin_src emacs-lisp

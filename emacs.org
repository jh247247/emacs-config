...
by some language switching keybinding or something. Really grinds my
gears. So, to fix this, make a new keybinding that adds a short
command to mark the buffer.
#+BEGIN_SRC emacs-lisp
  ;;(global-set-key (kbd "C-x SPC") 'set-mark-command)
#+END_SR
This was fixed with the inclusion of =god-mode=. No need to worry now.
